...
{
	initLights(pPart);
	raaTrafficSystem::addTarget(this); // adds the traffic light to the traffic system (static class) which holds a reord of all the dynamic parts in the system
}

TrafficLightFacarde::~TrafficLightFacarde()
{
	raaTrafficSystem::removeTarget(this); // removes the traffic light from the traffic system (static class) which holds a reord of all the dynamic parts in the system
}
...
void TrafficLightFacarde::setRedAndAmberTrafficLight()
{
